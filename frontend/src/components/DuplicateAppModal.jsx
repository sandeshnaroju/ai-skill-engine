...
import { Copy, X, Check, Building2, Box } from 'lucide-react';

export default function DuplicateAppModal({
  showModal,
  setShowModal,
  app,
  tenants,
  ...
  if (!showModal || !app) return null;

  const currentSourceTenantId = app.tenant_id || currentTenantId;
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await onDuplicate(app.id, selectedTenantIds, newAppName.trim());
      setShowModal(false);
    } catch (err) {
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '12px' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: '700', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}>
            <Copy size={18} color="var(--primary-cyan)" /> Duplicate App Container Across Workspaces
          </h3>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '12px' }}>
            ...
            <button
              ...
              disabled={submitting || selectedTenantIds.length === 0 || !newAppName.trim()}
