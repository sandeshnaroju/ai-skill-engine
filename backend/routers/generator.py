...
def generate_skill_content(
    ...
    system_prompt = (
        ...
        "    # HOW PARAMETERS ARE ROUTED AT RUNTIME (CRITICAL — READ CAREFULLY):\n"
        "    #\n"
        "    # ► GET / DELETE requests:\n"
        "    #     All 'parameters.properties' keys are automatically appended to the URL as query string parameters.\n"
        "    #     e.g. parameters: { city: \"London\" } → GET /endpoint?city=London\n"
        "    #\n"
        "    # ► POST / PUT / PATCH requests:\n"
        "    #     All 'parameters.properties' keys are sent as the JSON request BODY (Content-Type: application/json).\n"
        "    #     e.g. parameters: { username: \"alice\", amount: 100 } → POST /endpoint with body {\"username\": \"alice\", \"amount\": 100}\n"
        "    #     Do NOT put POST body fields in the URL. Define them ONLY under 'parameters.properties'.\n"
        "    #     The http executor will automatically send them as JSON body for POST/PUT/PATCH.\n"
        "    #\n"
        ...
        "5. For POST/PUT/PATCH tools, describe in the tool's 'description' field that the parameters will be sent as JSON body.\n"
    )
