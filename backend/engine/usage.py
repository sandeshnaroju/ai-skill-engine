...
def get_model_rates(db, tenant_id: str, model_name: str):
    ...
    if active_model_config:
        return (
            getattr(active_model_config, "input_rate", 0.0) if getattr(active_model_config, "input_rate", None) is not None else 0.0,
            getattr(active_model_config, "output_rate", 0.0) if getattr(active_model_config, "output_rate", None) is not None else 0.0,
            getattr(active_model_config, "audio_input_rate", 0.0) if getattr(active_model_config, "audio_input_rate", None) is not None else 0.0,
            getattr(active_model_config, "audio_output_rate", 0.0) if getattr(active_model_config, "audio_output_rate", None) is not None else 0.0,
        )
