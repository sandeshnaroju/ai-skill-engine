...
* **Remote Sandbox File Transfers**: If a file was generated inside a remote sandbox (such as Azure ACA or E2B), it does not exist locally yet. You **must first** call `download_sandbox_file` from the `sandbox_file_manager` skill, and then pass the resulting **unique local filename** (which has a UUID prefix, e.g. `7f650b7dac1a42b78ccfd985b16d83aa_animals.txt`) to the `filename` parameter of `upload_to_storage`. Do not use the original remote filename if a unique local copy has been created.
